...
export class RateLimit {
    ...
    enforceConnection(peer, eventType, windowMs) {
        const tabId =
            typeof peer?.tabId === "string" && peer.tabId.trim() ? peer.tabId.trim() : (peer?.id ?? "anonymous");
    ...
    enforcePlayerThrottle(tabId, eventType, windowMs) {
        this.#enforceRateLimit(`player:${ValidationUtils.requiredString(tabId, "tabId")}:${eventType}`, windowMs);
    }
    ...
    enforceRoomThrottle(roomKey, eventType, windowMs) {
        this.#enforceRateLimit(`room:${ValidationUtils.requiredString(roomKey, "roomKey")}:${eventType}`, windowMs);
    }
